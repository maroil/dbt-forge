...
This project is preparing the Python CLI package release `dbt-forge` version `0.3.1`.
...
Run `python3 scripts/release_assistant.py verify 0.3.1` on the exact commit that will become
...
3. Run `python3 scripts/release_assistant.py verify 0.3.1`.
4. Run `python3 scripts/release_assistant.py publish 0.3.1 --confirm`.
5. Let the script create and push tag `v0.3.1`.
6. Let GitHub Actions publish to PyPI from the tag.
7. Create the GitHub Release using the `0.3.1` changelog entry.
