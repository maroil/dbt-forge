...
This project is preparing the Python CLI package release `dbt-forge` version `0.4.2`.
...
Verified on 2026-03-14 from the local environment:
...
Run `python3 scripts/release_assistant.py verify 0.4.2` on the exact commit that will become
...
3. Run `python3 scripts/release_assistant.py verify 0.4.2`.
4. Run `python3 scripts/release_assistant.py publish 0.4.2 --confirm`.
5. Let the script create and push tag `v0.4.2`.
6. Let GitHub Actions publish to PyPI from the tag.
7. Create the GitHub Release using the `0.4.2` changelog entry.
